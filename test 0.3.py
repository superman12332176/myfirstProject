...
def show_visual_event(image_path, text_lines):
    ...
    current_row = 0
    ROWS_PER_FRAME = 1.2  # Faster image reveal (was 0.7)
    ...
    text_speed_current = 0.03  # Faster default speed (was 0.05)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # RPG-style: press any key to show full text instantly
                if text_index < len(full_text):
                    text_index = len(full_text)
                    display_text = full_text
                    current_row = IMAGE_HEIGHT  # Also complete image reveal
        ...
        # Typewriter logic
        if now - last_text_update > text_speed_current * 1000 and text_index < len(full_text):
            text_index += 1
